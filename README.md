# Central Coast Tree Services — rank-and-rent microsite
...
| Item | Value |
|---|---|
| Domain | `centralcoasttreeservice.com.au` (registered, GoDaddy fraud-pending) |
| Live URL (interim) | https://centralcoasttreeservice.netlify.app |
...
│   ├── officer.astro         # /wyong/
│   ├── berwick.astro         # /woy-woy/
│   ├── beaconsfield.astro    # /gosford/
│   ├── cardinia.astro        # /central-coast-region/ (shire-wide overview)
...
├── whisper.js                # "Pete's Lead from Central Coast Tree Services" played to contractor before patching caller through
...
| `TENANT_BRAND` | `Central Coast Tree Services` (default) | What plays after the label — full whisper is `"Pete's Lead. Pete's Lead from Central Coast Tree Services."` |
...
3. Submit to Google Search Console (`sc-domain:centralcoasttreeservice.com.au`).
4. Set up Google Business Profile (needs registered business name "Central Coast Tree Services" first).
